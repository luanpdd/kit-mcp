...
const CLAUDE_MD_FALLBACKS = {
  project: 'Project not yet initialized. Run /new-project to set up.',
...
function extractSectionContent(fileContent, sectionName) {
  const startMarker = `<!-- framework:${sectionName}-start`;
  const endMarker = `<!-- framework:${sectionName}-end -->`;
...
function buildSection(sectionName, sourceFile, content) {
  return [
    `<!-- framework:${sectionName}-start source:${sourceFile} -->`,
    content,
    `<!-- framework:${sectionName}-end -->`,
  ].join('\n');
}
...
function cmdWriteProfile(cwd, options, raw) {
  ...
  if (!outputPath) {
    outputPath = path.join(os.homedir(), '.claude', 'framework', 'USER-PROFILE.md');
...
function cmdGenerateDevPreferences(cwd, options, raw) {
  ...
  if (analysis.data_source === 'questionnaire') {
    stackBlock = 'Stack preferences not available (questionnaire-only profile). Run `/profile-user --refresh` with session data to populate.';
  ...
  let outputPath = options.output;
  if (!outputPath) {
    outputPath = path.join(os.homedir(), '.claude', 'commands', 'framework', 'dev-preferences.md');
  } else if (!path.isAbsolute(outputPath)) {
    outputPath = path.join(cwd, outputPath);
  }
...
function cmdGenerateClaudeProfile(cwd, options, raw) {
  ...
  const sectionLines = [
    ...
    '## Developer Profile',
    '',
    `> Generated by framework from ${dataSource}. Run \`/profile-user --refresh\` to update.`,
    '',
  ...
  if (fs.existsSync(targetPath)) {
    ...
    const startMarker = '<!-- framework:profile-start -->';
    const endMarker = '<!-- framework:profile-end -->';
...
function cmdGenerateClaudeMd(cwd, options, raw) {
  ...
  const sectionHeadings = {
    ...
    workflow: '## framework Workflow Enforcement',
  };
  ...
  if (existingContent === null) {
    ...
    for (const name of MANAGED_SECTIONS) {
      ...
      const hasMarkers = fileContent.indexOf(`<!-- framework:${name}-start`) !== -1;
    ...
    if (!options.auto && fileContent.indexOf('<!-- framework:profile-start') === -1) {
  ...
  let profileStatus;
  if (finalContent && finalContent.indexOf('<!-- framework:profile-start') !== -1) {
    if (action === 'created' || existingContent.indexOf('<!-- framework:profile-start') === -1) {
  ...
  if (profileStatus === 'placeholder_added') message += ' Run /profile-user to unlock Developer Profile.';
