 ...
 *   2. User-level defaults from ~/.framework/defaults.json (if present)
...
function buildNewProjectConfig(userChoices) {
  ...
  // Load user-level defaults from ~/.framework/defaults.json if available
  const globalDefaultsPath = path.join(homedir, '.framework', 'defaults.json');
  let userDefaults = {};
  try {
 ...
 * configured during /new-project). All remaining keys are filled from
 * hardcoded defaults and optional ~/.framework/defaults.json.
 *
