...
export async function detectReverse(targetId, opts = {}) {
  ...
  if (target.workflows) pending.push(scanWorkflows (candidates,             target.workflows, projectRoot, kit.workflows ?? [],    kitRoot));
...
async function scanWorkflows(candidates, capCfg, projectRoot, kitWorkflows, kitRoot) {
  const dir = path.join(projectRoot, capCfg.path);
  let entries;
  try { entries = await fs.readdir(dir, { withFileTypes: true }); }
  catch { return; }

  const ext = capCfg.extension || '.workflow.js';
  for (const e of entries) {
    ...
    const kitItem = kitWorkflows.find(x => x.fileBase === fileBase || x.name === fileBase);
    if (!kitItem) {
      // No canonical source — but if it carries our header without modification,
      // treat as a vestigial stub (kit dropped the workflow) rather than user content.
      if (isCleanWorkflowStub(destContent)) continue;
      candidates.push({
        kind: 'workflow', name: fileBase, target: capCfg.path, destPath,
        kitPath: path.join(kitRoot, 'workflows', `${fileBase}${ext}`),
        reason: 'new-in-ide',
        diffSummary: `+${destContent.length} bytes (no kit source)`,
      });
      continue;
    ...
    // strip+compare is the source of truth — `includes` would false-clean an
    // edit that appended text below the canonical body without touching markers.
    const stripped = stripWorkflowBoilerplate(destContent);
    if (normalize(stripped) === normalize(kitItem.content)) continue;

    candidates.push({
      kind: 'workflow', name: kitItem.name, target: capCfg.path, destPath,
      kitPath: kitItem.absPath,
      reason: 'modified-in-ide',
      diffSummary: summarizeDiff(kitItem.content, stripped),
    });
...
function isCleanWorkflowStub(content) {
  // Reserved for the rare orphan-stub case (kit dropped the workflow but our
  // header is still in the IDE tree). True only if BOTH the marker and the 3
  // header lines are present AND the body is exactly the canonical content
  // (caller handles the latter via strip+compare against kit).
  if (!content.includes(STUB_MARKER_JS)) return false;
  return /Canonical source:/.test(content) &&
         /Edit the source file in the kit/.test(content) &&
         /Generated by kit-mcp at/.test(content);
}

function stripWorkflowBoilerplate(content) {
  // Drop our 4-line header (STUB_MARKER_JS + 3 metadata comments + blank line)
  // so the comparison against canonical doesn't false-positive on the header.
  if (!content.includes(STUB_MARKER_JS)) return content;
  const lines = content.split(/\r?\n/);
  const out = [];
  let dropping = false;
  for (const line of lines) {
    if (line.includes(STUB_MARKER_JS)) { dropping = true; continue; }
    if (dropping) {
      if (/^\/\/\s*Canonical source:/.test(line) ||
          /^\/\/\s*Edit the source file/.test(line) ||
          /^\/\/\s*Generated by kit-mcp/.test(line)) continue;
